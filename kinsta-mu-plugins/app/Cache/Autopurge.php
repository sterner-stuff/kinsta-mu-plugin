...
final class Autopurge implements ArrayAccess, IteratorAggregate, Hookable
{
	...
	/** @return Traversable<string,Autopurgable> */
	...
	{
		add_filter('default_option_kinsta-autopurge-status', static fn () => null, PHP_INT_MAX);
	...
	public function update(string $key, bool $status): bool
	{
		if (! array_key_exists($key, $this->autopurgables)) {
			throw new RuntimeException(sprintf('The autopurge controller with key "%s" does not exist.', sanitize_key($key)));
		}
